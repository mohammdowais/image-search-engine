...
function App() {
  ...
  const [responseTime, setResponseTime] = useState(null); // NEW

  const handleImageChange = (file) => {
    setSelectedImage(file);
    setResults([]);
    setPage(1);
    setResponseTime(null); // reset timing
  };
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setResponseTime((end - start).toFixed(2)); // ms
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const end = performance.now(); // END TIMER
      setResponseTime((end - start).toFixed(2)); // update time
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-100 p-4 bg-gradient-to-b from-slate-900 to-cyan-900">
      <h1 className="flex text-4xl font-bold mb-10 mt-3 text-slate-400">
        Image to Image Search
      </h1>
      <h1 className="flex text-2xl font-bold mb-6 mt-3 text-slate-500">
        Drop an image below to find similar looking images.
      </h1>
